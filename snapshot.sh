...
main () {
  ...
  if [[ $CHECK -eq 1 ]]; then
    echo "[$(greenprint "found: $CHECK directory") -- $(yellowprint "zfs snapshot")]"
    ...
    for i in "${zfs_array[@]}"; do
      ...
      echo "$(cyanprint "$i") --> $(yellowprint "zfs snapshot -r $dir_path@$time")";
    ...
    echo "[$(greenprint "found: $CHECK directories") -- $(yellowprint "zfs snapshot")]"
    zfs_list=$(zfs list | awk '{print $5'} | grep $1/ | tr '\n' ' '| sed 's/ *$//')
    eval "zfs_array=($zfs_list)"
